...
오늘의 핵심 이슈 판별 (가장 중요):
- 먼저 fact_pack.events_top와 news_kr_session / news_overnight를 교차 검토해, 오늘 시장을 실제로 움직였거나 앞으로 움직일 '핵심 이슈' 2~4개를 선별하라.
- 핵심 이슈는 헤드라인 개수가 아니라 '시장 파급력 x 한국 증시 연결도'로 판단하라. driver_rank / impact_scope / korea_relevance_score / mention_count / source_count를 근거로 삼아라.
- 표면적 사건과 실제 시장 동인을 구분하라. 여러 기사가 같은 이슈를 가리키면 하나의 해석으로 묶고(연결·인과·파급), 반대로 관심은 많아도 가격을 못 움직인 이슈는 격을 낮춰라.
- 각 핵심 이슈는 (1) 무슨 일 (2) 왜 지금 중요한가 (3) 한국 증시로 이어지는 경로(수급/업종/금리/환율/밸류에이션 중 무엇) (4) 강도와 지속성 판단을 포함해 서술하라.
- 가격 반응이 뉴스와 정합적인지 점검하라. 뉴스는 강한데 가격 반응이 약하거나(선반영·피로), 뉴스는 약한데 가격이 크게 움직이면(다른 동인) 그 괴리를 짚어라.
- 확인되지 않은 인과는 단정하지 말고 '추정:'으로 표기하되, 무엇이 확인되면 판단이 바뀌는지도 한 줄로 남겨라.
...
6) top_drivers: 8~12개. event 단위 우선. events_top.driver_rank가 높은 항목을 우선 반영.
   - 상위 2~4개는 위 '오늘의 핵심 이슈'와 일치시키고, why_it_matters에는 한국 증시로 이어지는 경로(수급/업종/금리/환율/밸류에이션)와 강도·지속성 판단을 담아라.
   - 단순 헤드라인 재진술 금지. 여러 기사를 종합한 해석 한 문장 + 시사점 한 문장 구조로 써라.
   - sources에는 해당 이슈의 근거 news id를 채워라.
